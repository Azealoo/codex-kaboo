...
describe("install / uninstall", () => {
  it("builds the target from realpaths, installs the scheduler, then runs one sync", async () => {
    ...
    expect(target).toMatchObject({
      ...
      kabooHome: s.paths.home,
      codexHome: s.codexHome,
      uid: 501,
    }); // buildScheduleTarget realpaths execPath (nvm/homebrew symlinks)
    let synced = 0;
    const result = await runInstall(
      { systemd: false, json: false },
      {
        ...s.deps,
        runSync: async () => {
          synced += 1;
  ...
  it("refuses to install when not logged in", async () => {
    ...
    const result = await runInstall(
      { systemd: false, json: false },
      { ...s.deps, runSync: async () => emptyReport },
    );
...
describe("status", () => {
  it("reports config, homes, last sync and scheduler state", async () => {
    ...
    expect(report).toMatchObject({
      ...
      lastSync: { at: 5, ok: false, error: "boom" },
      filesTracked: 1,
      filesWithErrors: 1,
      cliVersion: "0.1.0",
    ...
    const missing = await runStatus({
      ...s.deps,
      scriptPath: path.join(s.root, "gone.js"),
      cliVersion: "0.1.0",
    });
  ...
  it("names a file parked after repeated identical failures", async () => {
    ...
    const state = emptyState();
    state.files["parked"] = {
      ...failedFile("/p/rollout-parked.jsonl", "boom"),
      failure: { count: 6, size: 1, mtimeMs: 1 },
    };
    state.files["retrying"] = {
      ...failedFile("/p/rollout-retrying.jsonl", "transient"),
      failure: { count: 2, size: 1, mtimeMs: 1 },
    };
    await writeState(s.paths, state);
    const report = await runStatus({ ...s.deps, cliVersion: "0.1.0" });
    expect(report.filesWithErrors).toBe(2);
    expect(report.filesParked).toEqual([
      { name: "rollout-parked.jsonl", failures: 6, error: "boom" },
    ]);
...
describe("doctor", () => {
  ...
  it("distinguishes a parked file from one still being retried in the state check", async () => {
    ...
    retrying.files["a"] = {
      ...failedFile("/p/a.jsonl", "transient 500"),
      failure: { count: 2, size: 1, mtimeMs: 1 },
    };
    ...
    parked.files["a"] = {
      ...failedFile("/p/a.jsonl", "day out of range"),
      failure: { count: 6, size: 1, mtimeMs: 1 },
    };
