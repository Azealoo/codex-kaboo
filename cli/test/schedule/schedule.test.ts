...
function mockSpawner(
  handler: (command: string, args: string[], input?: string) => SpawnResult | undefined,
) {
...
describe("launchd", () => {
  it("renders an escaped plist and installs via bootout/bootstrap/kickstart", async () => {
    ...
    const { spawner, calls } = mockSpawner((cmd, args) =>
      args[0] === "bootout" ? { code: 3, stdout: "", stderr: "not loaded" } : undefined,
    );
    ...
    const status = await launchdAdapter.status(
      t,
      mockSpawner(() => ({ code: 0, stdout: "state = running", stderr: "" })).spawner,
    );
  ...
  it("surfaces a launchctl kickstart failure during install", async () => {
    ...
    const { spawner } = mockSpawner((cmd, args) =>
      args[0] === "kickstart"
        ? { code: 1, stdout: "", stderr: "kickstart: could not start job" }
        : undefined,
    );
...
describe("cron", () => {
  it("renders the line and keeps exactly one marker block", () => {
    ...
    expect(line).toBe(
      `*/15 * * * * CODEX_KABOO_SCHEDULED=1 CODEX_HOME='/srv/codex' '/opt/node & co/bin/node' '/Users/me/.npm-global/lib/node_modules/codex-kaboo-cli/dist/codex-kaboo.js' sync --scheduled >> '/Users/me/.codex-kaboo/cron.log' 2>&1`,
    );
  ...
  it.skipIf(process.platform === "win32")(
    "passes $, a backtick and a backslash through /bin/sh unchanged",
    () => {
  ...
  it("installs through crontab -l / crontab - and reports status", async () => {
    ...
    const { spawner, calls } = mockSpawner((cmd, args, input) => {
      if (args[0] === "-l")
        return stored
          ? { code: 0, stdout: stored, stderr: "" }
          : { code: 1, stdout: "", stderr: "no crontab for me" };
      if (args[0] === "-") {
        stored = input ?? "";
  ...
  it("refuses a path containing a newline or carriage return instead of writing a broken crontab line", async () => {
    ...
    expect(() =>
      renderCronLine(target({ scriptPath: "/Users/me/weird\rpath/codex-kaboo.js" })),
    ).toThrow(/newline/i);
    expect(() => renderCronLine(target({ codexHome: "/srv/co\ndex" }))).toThrow(/crontab/i);
    expect(() => renderCronLine(target({ kabooHome: "/Users/me/.codex-kaboo\r" }))).toThrow(
      /newline/i,
    );
    expect(() => cronQuote("a\nb")).toThrow(/newline/i);
    ...
    const { spawner, calls } = mockSpawner((cmd, args) =>
      args[0] === "-l" ? { code: 1, stdout: "", stderr: "no crontab for me" } : undefined,
    );
    await expect(
      cronAdapter.install(target({ nodePath: "/opt/node\n/bin/node" }), spawner),
  ...
  it("distinguishes a missing crontab binary from an empty, not-yet-installed crontab", async () => {
    const missingBinary = mockSpawner((cmd, args) =>
      args[0] === "-l" ? { code: null, stdout: "", stderr: "spawn crontab ENOENT" } : undefined,
    );
    ...
    await expect(cronAdapter.install(target(), missingBinary.spawner)).rejects.toThrow(
      CrontabUnavailableError,
    );
    await expect(cronAdapter.uninstall(target(), missingBinary.spawner)).rejects.toThrow(
      CrontabUnavailableError,
    );

    // Contrast: crontab runs fine and reports none installed yet — a normal, healthy state, not
    // an error.
    const noCrontabYet = mockSpawner((cmd, args) =>
      args[0] === "-l" ? { code: 1, stdout: "", stderr: "no crontab for me" } : undefined,
    );
    expect(await cronAdapter.status(target(), noCrontabYet.spawner)).toEqual({
      installed: false,
      healthy: false,
    ...
    const timedOut = mockSpawner((cmd, args) =>
      args[0] === "-l" ? { code: null, stdout: "", stderr: "", timedOut: true } : undefined,
    );
...
describe("systemd", () => {
  it("renders unit files and enables the timer", async () => {
    ...
    expect(renderService(t)).toContain(
      `ExecStart="/opt/node & co/bin/node" "${t.scriptPath}" sync --scheduled`,
    );
    ...
    expect(calls.map((c) => c.args.join(" "))).toEqual([
      "--user daemon-reload",
      "--user enable --now codex-kaboo-sync.timer",
    ]);
  ...
  it("escapes backslash, quote and dollar in ExecStart, and leaves $ alone in Environment", () => {
    ...
    expect(service).toContain(
      String.raw`ExecStart="/opt/no\\de/bin/node" "/srv/we\"ird/$$HOME/back` +
        "`tick`" +
        '/codex-kaboo.js"',
    );
  ...
  it("refuses a path containing a newline or carriage return instead of writing a broken unit file", async () => {
    ...
    expect(() => renderService(target({ scriptPath: "/srv/we\rird/codex-kaboo.js" }))).toThrow(
      /newline/i,
    );
...
describe("schtasks", () => {
  it("renders a hidden VBS runner with doubled quotes and the schtasks arguments", async () => {
    const t = target({
      ...
      kabooHome: "C:\\Users\\me\\.codex-kaboo",
      homeDir: "C:\\Users\\me",
      codexHome: "D:\\codex",
    });
    ...
    expect(schtasksCreateArgs('wscript.exe //B //Nologo "C:\\x\\sync-hidden.vbs"')).toEqual([
      "/Create",
      "/F",
      "/SC",
      "MINUTE",
      ...
      TASK_NAME,
      "/TR",
      'wscript.exe //B //Nologo "C:\\x\\sync-hidden.vbs"',
    ]);
    ...
    const { spawner, calls } = mockSpawner((cmd, _args) =>
      cmd === "where"
        ? { code: 0, stdout: "C:\\Windows\\System32\\wscript.exe", stderr: "" }
        : undefined,
    );
    ...
    const status = await schtasksAdapter.status(
      { ...t, homeDir, kabooHome },
      mockSpawner(() => ({
        code: 0,
        stdout: "TaskName: \\codex-kaboo-sync\nStatus: Ready\n",
        stderr: "",
      })).spawner,
    );
  ...
  it("falls back to the PowerShell runner when wscript.exe is unavailable", async () => {
    ...
    const { spawner, calls } = mockSpawner((cmd) =>
      cmd === "where"
        ? {
            code: 1,
            stdout: "",
            stderr: "INFO: Could not find files matching the specified pattern.",
          }
        : undefined,
    );
  ...
  it("refuses a newline in kabooHome itself, for both the VBS and the .ps1 runner", async () => {
    ...
    const ps = mockSpawner((cmd) =>
      cmd === "where" ? { code: 1, stdout: "", stderr: "" } : undefined,
    );
...
describe("index", () => {
  it("picks the scheduler per platform and detects missing paths", async () => {
    ...
    expect(
      await checkTargetPaths(target({ nodePath: process.execPath, scriptPath: script })),
    ).toEqual([]);
    expect(
      await checkTargetPaths(
        target({ nodePath: path.join(dir, "missing-node"), scriptPath: script }),
      ),
    ).toEqual([path.join(dir, "missing-node")]);
  });
});
