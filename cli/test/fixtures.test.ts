...
describe("fixtures", () => {
  ...
  it("parses the small paginated CLI session (smoke reference)", async () => {
    ...
    expect(s).toMatchObject({
      sessionId: FX.paginatedSmall,
      threadId: FX.paginatedSmall,
      project: "project-a",
      source: "cli",
    ...
    expect(
      parsed.events.every(
        (e) => TokenEvent.safeParse(e).success && e.model === "gpt-5.6-sol" && e.effort === "xhigh",
      ),
    ).toBe(true);
  ...
  it("parses the exec session with two compactions", async () => {
    ...
    expect(s.toolCounts).toMatchObject({
      commandRead: 195,
      commandList: 5,
      commandSearch: 14,
      commandOther: 6,
  ...
  it("parses the legacy sub-agent file (no ordinals, event_msg messages)", async () => {
    ...
    const s = parsed.summary;
    expect(s).toMatchObject({
      sessionId: FX.legacySubagent,
      parentThreadId: FX.parent,
      isSubagent: true,
  ...
  it("ignores a trailing partial line and counts a corrupt line", async () => {
    ...
    expect(partial.parsed.summary).toMatchObject({
      responses: 1,
      turns: 1,
      completedTurns: 0,
      inProgress: true,
    ...
    expect(corrupt.parsed.summary).toMatchObject({
      lineCount: 6,
      parseErrors: 1,
      responses: 1,
      turns: 1,
  ...
  it("tolerates future wire types and prefers token_usage_record", async () => {
    ...
    expect(parsed.summary.tokens).toMatchObject({
      input: 300,
      cachedInput: 100,
      output: 20,
      reasoning: 5,
    ...
    expect(parsed.diagnostics.unknownTypes).toMatchObject({
      world_state: 1,
      inter_agent_communication: 1,
    });
