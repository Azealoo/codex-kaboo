...
const line = (t, type, payload, ordinal) =>
  JSON.stringify(
    ordinal === undefined
      ? { timestamp: iso(t), type, payload }
      : { timestamp: iso(t), ordinal, type, payload },
  );
...
const meta = (t, id, extra = {}) => ({
  id,
  timestamp: iso(t),
  cwd: "/redacted/project-c",
  originator: "codex-tui",
  source: "cli",
  cli_version: "0.150.1",
  history_mode: "paginated",
  git: { branch: "main" },
  base_instructions: { text: "<r:10>", provenance: { type: "model", model: "gpt-5.6-sol" } },
  ...extra,
});
const rateLimits = (used, t) => ({
  primary: { used_percent: used, window_minutes: 10080, resets_at: Math.floor(t / 1000) + 86400 },
  secondary: null,
  plan_type: "pro",
  limit_id: "weekly",
});
  ...
  const lines = [
    ...
    line(
      ...
      "event_msg",
      {
        type: "task_started",
        turn_id: "t1",
        started_at: Math.floor(t0 / 1000) + 1,
    ...
    line(
      ...
      {
        type: "token_count",
        info: { last_token_usage: usage(1200, 1000, 30, 10), model_context_window: 272000 },
        rate_limits: rateLimits(42.5, t0),
      },
      3,
    ),
  ];
  write(
    `sessions/2026/08/30/rollout-2026-08-30T20-00-00-${B1}.jsonl`,
...
const corruptLines = (id, t0) => [
  ...
  line(
    t0 + 1000,
    "event_msg",
    { type: "task_started", turn_id: "t1", started_at: Math.floor(t0 / 1000) + 1 },
    1,
  ),
  line(
    t0 + 1500,
    "turn_context",
    { turn_id: "t1", model: "gpt-5.6-luna", effort: "low", timezone: "Asia/Tokyo" },
    2,
  ),
  "{not json at all",
  line(
    ...
    {
      type: "token_count",
      info: { last_token_usage: usage(100, 40, 10, 5) },
      rate_limits: rateLimits(50, t0),
    },
    4,
  ),
  line(
    ...
    "event_msg",
    {
      ...
      turn_id: "t1",
      started_at: Math.floor(t0 / 1000) + 1,
      completed_at: Math.floor(t0 / 1000) + 3,
      duration_ms: 1500,
...
write(
  `sessions/2026/08/30/rollout-2026-08-30T21-00-00-${B2}.jsonl`,
  `${corruptLines(B2, at(2026, 8, 30, 21, 0, 0)).join("\n")}\n`,
);
write(
  `sessions/2026/08/31/rollout-2026-08-31T09-00-00-${B2}_${C1}.jsonl`,
  `${corruptLines(B2, at(2026, 8, 31, 9, 0, 0)).join("\n")}\n`,
);
...
// 3. Future wire types: world_state, token_usage_record (wins over token_count), McpToolCall, unknown item and type.
{
  ...
  const lines = [
    ...
    line(
      t0 + 1500,
      "turn_context",
      { turn_id: "t1", model: "gpt-5.7-future", effort: "high", timezone: "Europe/Berlin" },
      3,
    ),
    line(
      t0 + 2000,
    ...
    line(
      t0 + 2600,
      "event_msg",
      { type: "item_completed", item: { type: "Plan", id: "p1", text: "<r:5>" } },
      7,
    ),
    line(
      t0 + 2700,
      "response_item",
      { type: "function_call", name: "mcp__github__list_issues", arguments: "{}", call_id: "c1" },
      8,
    ),
...
// 4. zstd-compressed archived session (only when this Node has zstd; Node >= 22.15).
{
  ...
  const lines = [
    ...
    line(
      t0 + 1500,
      "turn_context",
      { turn_id: "t1", model: "gpt-5.6-sol", effort: "xhigh", timezone: "UTC" },
      2,
    ),
    line(
      t0 + 2000,
      "event_msg",
      { type: "token_count", info: { last_token_usage: usage(10, 0, 1, 0) }, rate_limits: null },
      3,
    ),
  ];
  if (typeof zlib.zstdCompressSync === "function") {
    write(
      `archived_sessions/2026/07/01/rollout-2026-07-01T08-00-00-${B4}.jsonl.zst`,
      zlib.zstdCompressSync(Buffer.from(`${lines.join("\n")}\n`)),
    );
  } else {
