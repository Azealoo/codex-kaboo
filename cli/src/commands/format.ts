...
export function formatSyncReport(report: SyncReport): string[] {
  const lines: string[] = [];
  for (const home of report.homes)
    lines.push(
      `codex home: ${home.path} ${home.exists ? `(${home.files} rollout files)` : "(missing)"}`,
    );
  for (const file of report.files) {
    if (file.action === "unchanged" && !report.dryRun) continue;
    const extra =
      file.action === "error" || file.action === "skipped"
        ? ` — ${file.reason ?? ""}`
  ...
  if (report.dryRun)
    lines.push(
      "dry run: nothing was sent and no state was written (use --json to inspect the exact payloads)",
    );
  return lines;
}
