...
describe("machines", () => {
  ...
  it("renames own machines only and validates the label", async () => {
    ...
    expect(
      await withUser(t, "alice").mutation(api.machines.rename, {
        machineId: "machine-1",
        label: "  work laptop ",
      }),
    ).toBeNull();
    ...
    await expect(
      withUser(t, "bob").mutation(api.machines.rename, { machineId: "machine-1", label: "x" }),
    ).rejects.toMatchObject({
      data: { code: "forbidden" },
    });
    await expect(
      withUser(t, "alice").mutation(api.machines.rename, { machineId: "machine-1", label: "   " }),
    ).rejects.toMatchObject({
      data: { code: "bad_label" },
    });
    await expect(
      withUser(t, "alice").mutation(api.machines.rename, { machineId: "nope", label: "x" }),
    ).rejects.toMatchObject({
      data: { code: "forbidden" },
    });
  });
